...
<div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
        ...
        <p class="text-sm text-gray-500">Isi form berikut untuk mencatat pelanggaran siswa</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-lg">
        <form method="POST" action="{{ route('inputpelanggaran.store') }}" enctype="multipart/form-data" class="p-8">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <!-- Selected Jenis -->
                        <div id="selectedJenis" class="hidden p-4 bg-red-50 border border-red-200 rounded-xl">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3 flex-1 min-w-0">
                                    <div class="w-10 h-10 bg-red-500 text-white rounded-xl flex items-center justify-center flex-shrink-0">
                                        <i class="fas fa-exclamation-triangle"></i>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-gray-800 truncate" id="jenisName"></p>
                                        <div class="flex items-center gap-3 mt-0.5">
                                            <span class="text-xs font-semibold text-red-600" id="jenisInfo"></span>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="bg-white rounded-2xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-primary/5 to-purple-600/5">
            <h3 class="font-bold text-gray-800 flex items-center gap-2">
                ...
                <span>Pilih Jenis Pelanggaran</span>
            </h3>
...
const jenisIdInput = document.getElementById('jenis_pelanggaran_id');
...
function searchJenisHandler() {
    ...
    if (filtered.length > 0) {
        ...
        filtered.forEach(jenis => {
            html += `
                <div class="px-4 py-3 hover:bg-red-50 cursor-pointer border-b border-gray-100 last:border-0 text-sm rounded-lg transition-colors" 
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <span class="text-xs font-semibold text-red-600">-${jenis.poin} poin</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        jenisDropdown.innerHTML = `
            <div class="p-8 text-center text-gray-500 text-sm">
                <i class="fas fa-gavel text-2xl mb-2 text-gray-300"></i>
                <p>Jenis pelanggaran tidak ditemukan</p>
            </div>
        `;
        jenisDropdown.classList.remove('hidden');
    }
...
function showAllJenis() {
    ...
    Object.keys(groupedJenis).sort().forEach(kategori => {
        ...
        groupedJenis[kategori].forEach(jenis => {
            html += `
                <div class="px-4 py-3 hover:bg-red-50 cursor-pointer border border-gray-200 rounded-xl text-sm transition-all hover:border-red-300 hover:shadow-sm" 
                     onclick="selectJenisFromModal(${jenis.id}, '${jenis.nama.replace(/'/g, "\\'")}', ${jenis.poin})">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3 flex-1 min-w-0">
                            <div class="w-8 h-8 bg-red-100 text-red-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                <i class="fas fa-exclamation-triangle"></i>
                        ...
                        <span class="text-xs font-bold text-red-600 ml-2">-${jenis.poin} poin</span>
                    </div>
                </div>
            `;
        });
...
function selectJenisFromModal(id, nama, poin) {
    selectJenis(id, nama, poin);
    closeAllJenisModal();
}
    ...
    if (html === '') {
        html = `
            <div class="text-center py-12 text-gray-500">
                ...
                <p class="text-sm font-medium">Tidak ditemukan jenis pelanggaran</p>
            </div>
        `;
    }
