    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
        ...
        <a href="{{ route('wali_kelas.dashboard') }}"
           class="sidebar-menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200
                  {{ request()->routeIs('wali_kelas.dashboard') 
            ...
            <i class="fas fa-home w-5 text-center flex-shrink-0 {{ request()->routeIs('wali_kelas.dashboard') ? 'text-primary' : 'text-gray-500' }}"></i>
            <span class="truncate">Dashboard</span>
            @if(request()->routeIs('wali_kelas.dashboard'))
        ...
        <a href="{{ route('wali_kelas.profil') }}"
           class="sidebar-menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200
                  {{ request()->routeIs('wali_kelas.profil*') 
                      ? 'bg-gradient-to-r from-primary/10 to-purple-600/10 text-primary border-l-4 border-primary shadow-sm' 
                      : 'text-gray-700 hover:bg-gray-100 hover:text-primary hover:translate-x-1 hover:shadow-sm' }}">
            <i class="fas fa-user w-5 text-center flex-shrink-0 {{ request()->routeIs('wali_kelas.profil*') ? 'text-primary' : 'text-gray-500' }}"></i>
        ...
        <a href="{{ route('wali_kelas.siswa.index') }}"
           class="sidebar-menu-item flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200
                  {{ request()->routeIs('wali_kelas.siswa*') 
                      ? 'bg-gradient-to-r from-primary/10 to-purple-600/10 text-primary border-l-4 border-primary shadow-sm' 
                      : 'text-gray-700 hover:bg-gray-100 hover:text-primary hover:translate-x-1 hover:shadow-sm' }}">
            <i class="fas fa-user-graduate w-5 text-center flex-shrink-0 {{ request()->routeIs('wali_kelas.siswa*') ? 'text-primary' : 'text-gray-500' }}"></i>
